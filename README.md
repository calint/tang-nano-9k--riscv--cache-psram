...
* [`configuration-apply.py`](https://github.com/calint/tang-nano-9k--riscv--cache-psram/blob/main/configuration-apply.py) generates include files with constants in `/os/src/`, `/emulator/src/`, `/src/` and clock constraint in `/tang_nano_9k.sdc`
...
[ ] uarttx: optimize away the cycle between go=1 and busy=1
    => cannot with only always_ff or running on posedge clk
